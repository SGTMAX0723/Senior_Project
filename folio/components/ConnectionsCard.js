    ...
    return (
        <div className="grid grid-cols-3 h-24 w-full items-center bg-white bg-cover border-b-2 hover:">
            <div className="relative flex h-full w-full justify-center items-center rounded-r-xl bg-cover" style={{backgroundImage: "url('https://i.ibb.co/FWggPq1/banner.png')"}}>
                <div className="absolute -right-10 flex h-20 w-20 items-center justify-center rounded-full border-[4px] border-white bg-pink-400">
                    <img className="h-full w-full rounded-full" src={`https://folio-database.fly.dev/api/files/_pb_users_auth_/${props.followers}/${props.followerAvatar}`} alt='user profile'/> 
                </div>
            </div>
