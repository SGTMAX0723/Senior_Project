...
function page() {
  ...
  async function updateEmail(data: any) {
    ...
    const newEmail = data.newemail // Replace with the desired new email
    ...
    <div className='grid grid-cols-1 gap-10 place-items-center min-h-screen'>
      <div className="flex flex-col items-center space-y-6">
        ...
        <form onSubmit={handleSubmit(updateEmail)} className="flex flex-col items-center space-y-4">
          ...
          <div>
          <label className="sr-only">Email address</label>
            <input {...register("newemail", {required: "Email is required"})} name="newemail" type="email" className="w-96 appearance-none rounded-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" placeholder="New Email address"></input>
            {errors.email && <p className="text-red-500 text-sm">{(errors.email.message as string)}</p>}
